...
@Component({
  selector: 'app-abonement-report',
  templateUrl: './abonement-report.component.html',
  styleUrls: ['./abonement-report.component.css']
})
export class AbonementReportComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.dashboardService.doShowTitle('Отчет по абонементам');
    this.getDay();
    this.getListGroup();

    this.listReport = this.timeReportService.listReportAbonement;
    if (this.timeReportService.dataReportAbonement.date_start !== 0) {
      this.dateRange[0] = new Date(this.timeReportService.dataReportAbonement.date_start);
      this.dateRange[1] = new Date(this.timeReportService.dataReportAbonement.date_end);
    }
  }
  ...
  getListGroup() {
    this.timeReportService.getListGroupForAbonement().subscribe(res => {
      this.listGroup = res;
  ...
  getReport(dataAbonement) {
    this.timeReportService.getReportAbonement(dataAbonement).subscribe(res => {
      this.listReport = res;
    });
  }
